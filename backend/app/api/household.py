...
from app.crud.household import get_all, get_by_id, create, update, delete, get_stats, batch_create, lock, unlock
...
@router.get("")
def get_all_api(skip: int = 0, limit: int = 100, natural_village_id: int = Query(None), admin_village_id: int = Query(None), search: str = Query(None), is_locked: int = Query(None), db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    return get_all(db, skip=skip, limit=limit, natural_village_id=natural_village_id, admin_village_id=admin_village_id, search=search, is_locked=is_locked)
